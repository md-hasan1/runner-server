    ...
    ws.on("message", async (data: string) => {
      try {
        ...
        switch (parsedData.event) {
          ...
          // Optional: In-memory throttle map (can be replaced with Redis for distributed systems)
          ...
          case "updateLatLng": {
            ...
            // Throttle updates: allow only one update every 5 seconds per driver
            const lastUpdate = driverLastUpdateMap.get(ws.userId);
            const now = Date.now();
            if (lastUpdate && now - lastUpdate < 5000) {
           
              return;
            }
            driverLastUpdateMap.set(ws.userId, now);
